...
class Frame:
    def __init__(self, polygon, opacity=1.0):
    ...
    def _get_middle_ground(self):
        ...
        points_string = ' '.join([f'{point.x*self.scale},{point.y*self.scale}' for point in self.points])
        svg_content = f'<polygon points="{points_string}" class="polygon" opacity="{self.opacity}"/>\n'
        # Desenha os pontos
        for i, point in enumerate(self.points):
            vertex_class = self.points_colors[i]
            svg_content += f'<circle cx="{point.x*self.scale}" cy="{point.y*self.scale}" r="5" class="{vertex_class}_point opacity="{self.opacity}"/>\n'

        return svg_content

    # Colore um ponto
    def set_point_color(self, idx, new_color):
